...
def main():
    TARGET_VIDEO_NAME = "IF2001_3_1_1423111761_0.mp4.mp4"
    ...
    if len(matches) == 0:
        raise FileNotFoundError(
            f"비디오를 찾지 못했습니다: {TARGET_VIDEO_NAME}"
        )
    ...
    if detected:
        print("[TRUE JSON]", os.path.join(RESULT_TRUE_ROOT, f"{video_id}.json"))
        print("[TRUE VIDEO]", os.path.join(RESULT_TRUE_ROOT, f"{video_id}_gaze_result.mp4"))
    else:
        print("[FALSE JSON]", os.path.join(RESULT_FALSE_ROOT, f"{video_id}.json"))
        print("[FALSE VIDEO]", os.path.join(RESULT_FALSE_ROOT, f"{video_id}_gaze_result.mp4"))
