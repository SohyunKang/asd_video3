...
# =========================
FOLD_DIR = Path(
    "/home/sohyunkang/asd_video3/experiments/annotated_labels_timesformer_20260707_104808/fold_1"
)

VAL_CLIP_CSV = FOLD_DIR / "inference_clip_df.csv"
PRED_CSV = FOLD_DIR / "inference_predictions.csv"
...
CLIP_DURATION = float(CFG.get("clip_duration", 1.0))
STRIDE = float(CFG.get("stride", 0.5))
MAX_CLIPS_PER_VIDEO = int(CFG.get("num_clips_per_video", 30))

print("[INFO] config:", CONFIG_PATH)
print("[INFO] clip_duration:", CLIP_DURATION)
print("[INFO] stride:", STRIDE)
print("[INFO] max_clips_per_video:", MAX_CLIPS_PER_VIDEO)
...
def apply_result_filter(frame_bgr, true, pred, prob=None, alpha=0.35):
    true = int(true)
    pred = int(pred)

    # inference mode: true label 없음
    if true < 0:
        if pred == 1:
            color = (0, 0, 255)
            text = "Pred EC"
        ...
        color = (128, 128, 128)
        text = f"Unknown true={true}, pred={pred}"
...
def extract_emotion_events(json_data):
    ...
    for frame_obj in json_data:
        ...
        timestamp_ms = frame_obj.get("timestamp_ms")
        if timestamp_ms is None:
            continue

        time_sec = float(timestamp_ms) / 1000.0

        emotion_list = frame_obj.get("emotion", [])
...
def get_emotion_for_clip(emotion_events, clip_start, clip_end):
    ...
    for e in emotion_events:
        ...
        if t is None or emotion is None or prob is None:
            continue
    ...
    mean_score = (
        score_df
        .groupby("emotion")["prob"]
        .mean()
        .sort_values(ascending=False)
    )

    best_emotion = mean_score.index[0]
    best_prob = float(mean_score.iloc[0])

    return best_emotion, best_prob
...
def build_raw_clip_df_from_video_metadata(patient_df, clip_duration=0.4, stride=0.2):
    rows = []
    ...
    for _, row in video_meta.iterrows():
        fps = float(row["fps"])
        saved_n_frames = int(row["saved_n_frames"])
        ...
        while start + clip_duration <= max_time:
            ...
            new_row = row.copy()
            new_row["clip_start"] = start
            new_row["clip_end"] = end
...
all_clip_df["patient_id"] = all_clip_df["patient_id"].astype(str).str.strip()
all_clip_df["video_id"] = all_clip_df["video_id"].astype(str).str.strip()
...
for patient_id, patient_df in val_df.groupby("patient_id"):
    ...
    # old clip csv인 경우
    if (
        "clip_start" in all_clip_df.columns
        and "clip_end" in all_clip_df.columns
    ):
        ...
        # =========================
        # raw도 inference와 동일하게 제한
        # 1) 호명 이후 inference_clip_df 기준과 동일한 video/time만 사용
        # 2) max_clips_per_video 적용
        # =========================
        used_keys = set(
            zip(
                patient_df["video_id"].astype(str),
                patient_df["clip_start"].round(6),
                patient_df["clip_end"].round(6),
            )
        )

        raw_patient_df["clip_start_round"] = pd.to_numeric(
            raw_patient_df["clip_start"], errors="coerce"
        ).round(6)

        raw_patient_df["clip_end_round"] = pd.to_numeric(
            raw_patient_df["clip_end"], errors="coerce"
        ).round(6)

        raw_patient_df = raw_patient_df[
            raw_patient_df.apply(
                lambda r: (
                    str(r["video_id"]),
                    r["clip_start_round"],
                    r["clip_end_round"],
        ...
        raw_patient_df = (
            raw_patient_df
            .drop(columns=["clip_start_round", "clip_end_round"], errors="ignore")
            .sort_values(["video_id", "clip_start"])
            .groupby("video_id", group_keys=False)
            .head(MAX_CLIPS_PER_VIDEO)
            .reset_index(drop=True)
        )

        print(
            f"[INFO] raw clips after same inference filter: "
            f"patient={patient_id}, n={len(raw_patient_df)}"
        )

    # 새 video metadata인 경우
    else:
        raw_patient_df = build_raw_clip_df_from_video_metadata(
            patient_df,
    ...
    for _, row in patient_df.iterrows():
        ...
        arr = np.load(row["video_npy_path"], mmap_mode="r")
        ...
        center_idx = np.clip(center_idx, 0, len(arr)-1)
